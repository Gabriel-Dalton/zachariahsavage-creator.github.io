...
/** Full-gallery-only: first section, keyed paths (not `N_` filenames) get their own group. */
...
FULL_GALLERY_PINNED_SHOW.sources.forEach((path) => {
  fullGalleryCustomGroupByPath.set(path, {
    label: FULL_GALLERY_PINNED_SHOW.id,
    order: FULL_GALLERY_PINNED_SHOW.order,
  });
});
...
function setupScrollReveal() {
  ...
  const revealSequence = [
    { selector: ".gallery", variant: "soft", delay: 0 },
    { selector: ".home-bio-image", variant: "soft", delay: 70 },
    { selector: ".home-intro", variant: null, delay: 130 },
    { selector: ".onepage-section__heading", variant: "soft", delay: 0 },
    { selector: ".portfolio-controls", variant: "soft", delay: 50 },
    { selector: "#gallery-grid", variant: "soft", delay: 100 },
    { selector: ".portfolio-toggle", variant: "soft", delay: 140 },
    { selector: ".contact__heading", variant: "soft", delay: 0 },
    { selector: ".contact__intro", variant: "soft", delay: 65 },
    { selector: ".contact__form", variant: "soft", delay: 120 },
    { selector: ".contact__social", variant: "soft", delay: 175 },
  ];
...
function measureAndPosition(rowsState) {
  ...
  rowsState.forEach((rowState, rowIndex) => {
    ...
    const targetWidth = viewportWidth + itemsForRow.length * (itemW + spacing);
    const tilesA = [];
    let seq = 0;
    let segmentWidthPx = 0;
    while (segmentWidthPx < targetWidth) {
      const { item, mediaIndex } = itemsForRow[seq % itemsForRow.length];
    ...
    if (W > 0) {
      rowState.trackScroll = positiveModulo(rowState.trackScroll || 0, W);
    } else {
      rowState.trackScroll = 0;
    }
    const phase = W > 0 ? positiveModulo(rowState.trackScroll, W) : 0;
...
function startFlowAnimation(rowsState) {
  ...
  function step(now) {
    ...
    rowsState.forEach((rowState, rowIndex) => {
      ...
      if (!rowState.trackElement || !(W > 0)) return;

      const speed = (baseSpeeds[rowIndex] || baseSpeeds[0]) * flowMultiplier * flowDirection;
      // Positive speed used to increase per-tile x (drift right). With -mod(phase) on the track,
      // decreasing scroll moves the strip right; keep that mapping.
      rowState.trackScroll -= speed * dt;
      const phase = positiveModulo(rowState.trackScroll, W);
      rowState.trackElement.style.transform = `translate3d(${-phase}px, 0, 0)`;
    });
...
function renderJustifiedPortfolioByNamePrefix(grid, tiles) {
  ...
  sorted.forEach(([label, group]) => {
    ...
    const preserveSourceOrder = label === FULL_GALLERY_PINNED_SHOW.id;
    const orientedTiles = preserveSourceOrder ? [...group.tiles] : orderTilesByOrientation(group.tiles);
    let leadGroup;
    let secondGroup;
    if (preserveSourceOrder) {
      leadGroup = orientedTiles;
      secondGroup = [];
    } else {
      const { landscapes, portraits } = splitTilesByOrientation(orientedTiles);
      leadGroup = landscapes.length ? landscapes : portraits;
      secondGroup = landscapes.length ? portraits : [];
    }
...
function resolveIndexHashScrollTarget(hashId) {
  ...
  if (id === "gallery-section") {
    return document.getElementById("gallery-section-heading") || document.getElementById("gallery-section");
  }
...
function setupHireAvailabilityPill() {
  ...
  function scrollToContact() {
    const instant = window.matchMedia?.("(prefers-reduced-motion: reduce)")?.matches;
    // If the user is already at/inside the contact region, avoid triggering
    // a smooth scroll that "snaps" the page while they're trying to read/scroll.
    const rect = contactTarget.getBoundingClientRect();
    const headerPad = 160;
    const isNearTop = rect.top >= -24 && rect.top <= 24;
    // Only skip if the heading is actually parked under the header (user is already in contact).
    const isClearlyReadingContact =
      rect.top >= -20 && rect.top <= headerPad && rect.bottom > 80;
    if (isNearTop || isClearlyReadingContact) return;
  ...
  // Mobile: ignore stray clicks after big scroll drags; do not require `pendingTap`
  // (iOS can fire `click` without a reliable `pointerdown` on the same button).
  let gestureActive = false;
  let lastPointerType = "";
  let tapStartX = 0;
  let tapStartY = 0;
  let tapStartScrollTop = 0;
  let tapMoved = false;
  const TAP_MOVE_PX = 14;
  const TAP_SCROLL_PX_MOUSE = 48;
  const TAP_SCROLL_PX_TOUCH = 140;
  ...
  function onActionPointerDown(e) {
    ...
    tapStartX = e.clientX;
    tapStartY = e.clientY;
    tapStartScrollTop = getPageScrollTop();
  }
  ...
  function onActionPointerMove(e) {
    ...
    if (Math.hypot(dx, dy) > TAP_MOVE_PX) tapMoved = true;
  }

  function onActionPointerCancel() {
    gestureActive = false;
    tapMoved = false;
  }
  ...
  actionBtn.addEventListener("click", (e) => {
    ...
    if (gestureActive) {
      ...
      const maxScroll = isTouchLike ? TAP_SCROLL_PX_TOUCH : TAP_SCROLL_PX_MOUSE;
      const scrollDelta = Math.abs(getPageScrollTop() - tapStartScrollTop);
      if (tapMoved || scrollDelta > maxScroll) {
        gestureActive = false;
        tapMoved = false;
        return;
      }
  ...
  actionBtn.addEventListener("pointerdown", onActionPointerDown);
  actionBtn.addEventListener("pointermove", onActionPointerMove);
  actionBtn.addEventListener("pointercancel", onActionPointerCancel);
