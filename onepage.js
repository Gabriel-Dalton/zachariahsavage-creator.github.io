...
function setupScrollReveal() {
  ...
  const revealSequence = [
    ...
    { selector: ".home-start-btn", variant: "soft", delay: 140 },
...
window.addEventListener("load", () => {
  ...
  setTimeout(runFlowMeasure, 60);
  // iOS Safari can report a transient viewport before first interaction.
  // Re-measure shortly after paint to avoid "square first, then pop" flow tiles.
  setTimeout(runFlowMeasure, 260);
  ...
  let lastMeasuredVvHeight = Math.round(
    window.visualViewport?.height || window.innerHeight || 0
  );
  window.addEventListener("resize", () => {
    const nextWidth = window.innerWidth;
    const nextVvHeight = Math.round(
      window.visualViewport?.height || window.innerHeight || 0
    );
    const widthChanged = Math.abs(nextWidth - lastMeasuredWidth) >= 2;
    const viewportHeightChanged = Math.abs(nextVvHeight - lastMeasuredVvHeight) >= 12;
    // iOS can change visual viewport height as browser chrome settles; remeasure once that shift is meaningful.
    if (!widthChanged && !viewportHeightChanged) return;
    lastMeasuredWidth = nextWidth;
    lastMeasuredVvHeight = nextVvHeight;
  ...
  if (window.visualViewport) {
    window.visualViewport.addEventListener(
      ...
      () => {
        clearTimeout(resizeTimeout);
        resizeTimeout = setTimeout(() => measureAndPosition(rowsState), 120);
      },
      { passive: true }
    );
  }
